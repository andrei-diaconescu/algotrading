class Trade:
    def __init__(self, entry_price: float, side: str, open_time, position_size=float(1), **kwargs) -> None:
    ...
    def set_stop_loss(self, sl_price: float):
        ...
        self.sl_percentage = abs(
            self.entry - self.stop_loss) * 100 / self.entry

    def set_take_profit_price(self, tp_price: float):
        self.check_price_value(tp_price)
        self.take_profit = round(tp_price, 2)
        self.tp_percentage = abs(
            self.entry - self.take_profit) * 100 / self.entry

    def set_price_risk(self):
        self.price_risk = abs(
            self.take_profit - self.entry) / self.risk_to_reward
    ...
    def set_close_time(self, close_time):
        ...
        self.trade_duration = (
            self.close_time - self.open_time).total_seconds() / 60

    def to_dict(self) -> dict:
        trade_dict = {"open_time": self.open_time, "close_time": self.close_time, "duration": self.trade_duration,
                      "side": self.side,
                      "entry": self.entry,
                      "stop_loss": self.stop_loss, "sl_perc": self.sl_percentage, "take_profit": self.take_profit,
                      "tp_perc": self.tp_percentage, "risk_to_reward": self.risk_to_reward, "drawdown": self.drawdown,
                      "peek": self.peek, "result": self.result}
        return trade_dict
        ...
        if self.result is None:
            ...
            if self.result is None:
                ...
                if self.result == "profit":
                    self.drawdown = - \
                        (abs(self.calc_current_risk(self.drawdown_price)))
...
def calc_long_take_profit(long_entry_price, sar_value):
    return long_entry_price + 2 * (long_entry_price - sar_value)


def calc_short_take_profit(short_entry_price, sar_value):
    return short_entry_price - 2 * (sar_value - short_entry_price)


def calc_take_profit(trend, entry_price, sar_value):
    if trend == "LONG":
        return calc_long_take_profit(entry_price, sar_value)
    return calc_short_take_profit(entry_price, sar_value)
